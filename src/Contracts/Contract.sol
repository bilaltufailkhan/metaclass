...
contract block24 is ERC20Detailed, Ownable {
    ...
    string public _name = "block24";
    string public _symbol = "b24";
    ...
    uint256 public blockInsuranceFundFee = 10;
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(blockInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public blockInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("B24", "B24", uint8(DECIMALS)) Ownable() {
        ...
        blockInsuranceFundReceiver = 0x0827916234dd165eED8335BC9b5d72ed5fDeAa45;
    ...
    function takeFee(
        ...
        uint256 _blockInsuranceFundFee = blockInsuranceFundFee;
        uint256 _firePitFee = firePitFee;

        if (recipient == pair) {
            _totalFee = totalFee.add(sellFeet).add(sellFeef).add(sellFees).add(
                sellFeel
            );
            _treasuryFee = treasuryFee.add(sellFeet);
            _liquidityFee = liquidityFee.add(sellFeel);
            _blockInsuranceFundFee = blockInsuranceFundFee.add(sellFees);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(blockInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(blockInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(blockInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(blockInsuranceFundFee).div(
                treasuryFee.add(blockInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no B24 token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _blockInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        blockInsuranceFundReceiver = _blockInsuranceFundReceiver;
        firePit = _firePit;
    }
